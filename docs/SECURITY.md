...
| 클라이언트 JWT             | DB에는 bcrypt 해시만 저장 (rotate 시 비교용). 클라이언트 사본은 OS keychain (v0.3 P2 — macOS Keychain / Windows Credential Manager / Linux libsecret). 키체인 unavailable 시 `tauri-plugin-store` plain JSON 폴백 + UI 경고 (`docs/SELF-HOSTING.md` §3.1) |
