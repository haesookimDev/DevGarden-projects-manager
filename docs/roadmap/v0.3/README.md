...
| P2  | [Client JWT OS keychain](./P2-jwt-keychain.md)           | `tauri-plugin-store` plain JSON → OS keychain (`keyring` crate) + migration   | —      | ✅ 완료 (PR #124–#128)                               |
| P3  | [OAuth e2e + HTTPS dev](./P3-oauth-e2e.md)               | mkcert 자체 CA + GitHub OAuth mock + Playwright HTTPS context 로 실 OAuth e2e | —      | ✅ 완료 (PR #129–#132, HTTPS scope dropped to v0.4+) |
...
- [x] P1~P4 마일스톤의 각 acceptance 항목 완료. (P3 의 HTTPS dev 항목은 v0.4+ 로 이관 — 자세한 내용은 P3-oauth-e2e.md §2.1)
- [x] `docs/SELF-HOSTING.md` 가 Redis 옵션 + keychain 동작 + metrics endpoint 를 다룸.
...
- [x] CI 5 jobs 모두 green + PR-time docker build smoke green + nightly tauri build smoke green, 누적 테스트 ≥ 500 cases (509).
